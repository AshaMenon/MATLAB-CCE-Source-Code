function [outputs, errorCode] = pythonFunctionTemplate(parameters,inputs)
    %PYTHONFUNCTIONTEMPLATE Function to call the Python *functionName function
    try
        % Setup
        pe = pyenv;
        if pe.Status ~= 'Loaded'
        ...
        timestampIdx = find(contains(inputFields,'Timestamps'));
        ...
        outputPy = eval('py.moduleName.functionName(parameters, inputs)');
        ...
        timestampIdx = find(contains(outputFields,'Timestamp'));
        if ~isempty(timestampIdx)
