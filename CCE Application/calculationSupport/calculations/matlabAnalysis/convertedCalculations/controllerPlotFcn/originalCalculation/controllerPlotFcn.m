function result = controllerPlotFcn(description, varargin)
%CONTROLLERPLOTFCN Control analysis for furnace OPM Units
%
%   result = controllerPlotFcn(description,...
%            'SensorDataPV',sensorDataPV,...
%            'SensorDataSP',sensorDataSP, ...
%            'SensorDataMV',sensorDataMV,...
%            'SensorDataAutoman',sensorDataAutoman,...
%            'TuningParameterP',tuningParameterP,...
%            'TuningParameterI',tuningParameterI,...
%            'TuningParameterD',tuningParameterD,...
...
%
% DESCRIPTION:
% Produce a control comparison of data for OPM Sensor objects. All samples
% are used in the control plot analysis. Only samples that are valid in all
% sensors for Missing Data, Not Running, Not Updating or ROC are used in
% the control error evaluation. A sample is valid if it's quality value is
% 'Good', 'High', and 'Low'.
%
% Controller Status: (1) Instrument Fault; (2) Manual; (3) Internal Error; (4) Unhealthy; (5)Healthy
% Controller Root Cause: (1) Saturation; (2) Disturbance/Slow Control; (3) Disturbance/Fast Inner Loop; (4) Valve Stiction/Pump Problem; (5) Cause Unknown
%
% INPUT:
%       description: char; description of analysis
% 
%   REQUIRED PARAMETERS:
%    'SensorDataPV': struct; a sensor data stucture, containing process
%                    value data including instrument high & low limits
%
%    'SensorDataSP': struct; a sensor data stucture, containing set point
%                    data. 
%
%   OPTIONAL PARAMETERS:
%     'SensorDataMV': struct; a sensor data stucture, containing manipulated
%                     variable data (viz. controller output data)
%
% 'SensorDataAutoman':struct, a sensor data stucture, containing automatic
%                     and manual control state data
%
%     'TuningParameterP': struct; a sensor data stucture, containing tuning
%                     parameter P data
%
%     'TuningParameterI': struct; a sensor data stucture, containing tuning
%                     parameter I data
%
%     'TuningParameterD': struct; a sensor data stucture, containing tuning
%                     parameter D data
%
% 'controllerConstraint': string; define if PV should be:
%                           on SP (none - default)
%                           below SP (upper)
%                           above SP (lower)
%
%         'threshold': numeric; control error threshold [% of PV range]
%
%         'tintegral': numeric; controller integral time [seconds]
%
%          'tagNames': string; export data tagName
%
%            'report': char; type of report: "full" or "exec" (default = "full")
%
%                'link': struct, reference analysis unit struct tree.  Only
%                        used if 'report' = "exec"
%
% RETURN:
%       result: ap.AnalysisResult; analysis result
%
...
%try
    ...
    % Prepare the output
    result  = ap.AnalysisResult(sprintf('Control plot (for combined data): %s',description)); % Create container for analysis results
    ...
    goodQualityVal      = ap.DataQuality.opmqualityval('good');
    notRunningQualityVal= ap.DataQuality.opmqualityval('not running');
...
%                         end
    ...
    % Plot graphs (PV, SP, MV, AutoMan)
    f = opmblankfigure;
    width = get_axes_width(gca(f));
    result.TimeSpan.Result(1).ImageH = f; % Store blank OPM figure in results

    plotTimeStampsActual = plotTimeStamps;
    plotTimeStamps = (1:size(plotTimeStamps,1))';

    subplot(3,1,1);
    % Plot process value (blue) and set point (red) for this timespan
    [plotDates, plotData] = reduce_to_width(plotTimeStamps,plotCntrlData,width,[-inf inf]);
%     plot(plotTimeStamps,plotCntrlData(:,1),'b',plotTimeStamps,plotCntrlData(:,2),'r');
    plot(plotDates,plotData(:,1),'b',plotDates,plotData(:,2),'r');
    hold on
    % Plot not-good quality data (black)
    [plotDates, plotData] = reduce_to_width(plotTimeStamps(plotQuality~=goodQualityVal,1),plotCntrlData(plotQuality~=goodQualityVal,1),width,[-inf inf]);
%     plot(plotTimeStamps(plotQuality~=goodQualityVal,1), plotCntrlData(plotQuality~=goodQualityVal,1), 'marker', '.', 'markersize', 10, 'linestyle', 'none', 'color', 'k');
    plot(plotDates, plotData, 'marker', '.', 'markersize', 10, 'linestyle', 'none', 'color', 'k');
    [plotDates, plotData] = reduce_to_width(plotTimeStamps(plotQuality==notRunningQualityVal,1),plotCntrlData(plotQuality==notRunningQualityVal,1),width,[-inf inf]);
%     plot(plotTimeStamps(plotQuality==notRunningQualityVal,1), plotCntrlData(plotQuality==notRunningQualityVal,1), 'marker', '.', 'markersize', 10, 'linestyle', 'none', 'color', [0.93 0.93 0.93]);
    plot(plotDates, plotData, 'marker', '.', 'markersize', 10, 'linestyle', 'none', 'color', [0.93 0.93 0.93]);
    % Plot "in-control-data" (green)
    dataL = 0;
    for k = 1:numTS-1
        dataL = dataL + timespanData.length{k};
        if abs(diff([timespanData.timeStamps{k}(end) timespanData.timeStamps{k+1}(1)])) > datenum(0,0,0,0,0,timespanData.timeStep{k})
            plot(repmat(plotTimeStamps(dataL),2,1),yRange_1,'r','LineWidth',2);
        else
            plot(repmat(plotTimeStamps(dataL),2,1),yRange_1,'r--');
        end
    end
    % Add the OPMCondition patches if present
    if ~isempty(condition)
        graphYlims = get(gca,'YLim');
%         colList = {'b' 'g' 'r' 'c' 'm' 'y' 'k'};
        colList = hsv(length(condition.unique)); % Colour vector to ensure different graph colours - linked to normcompare colour list
        if length(condition.unique) == 2, colList = [198 12 48; 0 39 118]/255; end % ANGLO COLOUR SCHEME
        for i = 1:length(condition.unique)
            for j = 1:length(condition.ind(i).start)
                patch([condition.ind(i).start(j) condition.ind(i).end(j) condition.ind(i).end(j) condition.ind(i).start(j) condition.ind(i).start(j)],[graphYlims(1) graphYlims(1) graphYlims(2) graphYlims(2) graphYlims(1)],colList(i,:),'FaceAlpha',0.3,'EdgeColor',colList(i,:),'EdgeAlpha',0,'LineWidth',1);
            end
        end
    end
    set(gca,'YLim', yRange_1, 'XLim', plotTimeStamps([1, end]));
    if max(get(gca, 'YTick')) > 10000, set(gca,'YTick',get(gca, 'YTick'), 'YTickLabel', num2str(get(gca, 'YTick')')); end
    opmdatetick2(gca, 'single',plotTimeStampsActual);
    ylabel('PV / SP');
    dtFmt = smartdateformat(plotTimeStamps([1, end]));
    title(sprintf('Combined data (PV - blue; SP - red; Oscillation - green) - (%.1f%% of data plotted)\n%s - %s', ...
        length(plotCntrlData(plotQuality==goodQualityVal,1))./length(plotCntrlData(:,1)).*100, ...
        datestr(plotTimeStampsActual(1), dtFmt), datestr(plotTimeStampsActual(end), dtFmt)));
    grid on;

    subplot(3,1,2);
    [plotDates, plotData] = reduce_to_width(plotTimeStamps,plotCntrlData,width,[-inf inf]);
%     plot(plotTimeStamps,plotCntrlData(:,3),'g',plotTimeStamps,plotCntrlData(:,4)*yRange_3(2)*.9+yRange_3(2)*.05,'m');
    plot(plotDates,plotData(:,3),'g',plotDates,plotData(:,4)*yRange_3(2)*.9+yRange_3(2)*.05,'m');
    hold on
    [plotDates, plotData] = reduce_to_width(plotTimeStamps(plotQuality~=goodQualityVal,1),plotCntrlData(plotQuality~=goodQualityVal,3),width,[-inf inf]);
%     plot(plotTimeStamps(plotQuality~=goodQualityVal,1), plotCntrlData(plotQuality~=goodQualityVal,3), 'marker', '.', 'markersize', 10, 'linestyle', 'none', 'color', 'k');
    plot(plotDates, plotData, 'marker', '.', 'markersize', 10, 'linestyle', 'none', 'color', 'k');
    [plotDates, plotData] = reduce_to_width(plotTimeStamps(plotQuality==notRunningQualityVal,1),plotCntrlData(plotQuality==notRunningQualityVal,3),width,[-inf inf]);
%     plot(plotTimeStamps(plotQuality==notRunningQualityVal,1), plotCntrlData(plotQuality==notRunningQualityVal,3), 'marker', '.', 'markersize', 10, 'linestyle', 'none', 'color', [0.93 0.93 0.93]);
    plot(plotDates, plotData, 'marker', '.', 'markersize', 10, 'linestyle', 'none', 'color', [0.93 0.93 0.93]);
    dataL = 0;
    for k = 1:numTS-1
        dataL = dataL + timespanData.length{k};
        if abs(diff([timespanData.timeStamps{k}(end) timespanData.timeStamps{k+1}(1)])) > datenum(0,0,0,0,0,timespanData.timeStep{k})
            plot(repmat(plotTimeStamps(dataL),2,1),yRange_3,'r','LineWidth',2);
        else
            plot(repmat(plotTimeStamps(dataL),2,1),yRange_3,'r--');
        end
    end
    % Add the OPMCondition patches if present
    if ~isempty(condition)
...
%         colList = {'b' 'g' 'r' 'c' 'm' 'y' 'k'};
    ...
    set(gca,'YLim', yRange_3, 'XLim', plotTimeStamps([1, end]));
    if max(get(gca, 'YTick')) > 10000, set(gca,'YTick',get(gca, 'YTick'), 'YTickLabel', num2str(get(gca, 'YTick')')); end
    opmdatetick2(gca, 'single',plotTimeStampsActual);
    ylabel('OP / Auto');
    title(sprintf('Combined data (OP - green; Auto - magenta)'));
    grid on;

    % Plot graphs (Control error - red; Threshold - green)
    subplot(3,1,3);
    if strcmpi(controllerConstraint,'upper')
        [plotDates, plotData] = reduce_to_width(plotTimeStamps,plotCntrlData(:,5),width,[-inf inf]);
        [plotDates2, plotData2] = reduce_to_width(plotTimeStamps,-repmat(normCtrlErrorThreshold,size(plotCntrlData(:,5),1),1),width,[-inf inf]);
        plot(plotDates,plotData,'r',plotDates2,plotData2,'g');
    elseif strcmpi(controllerConstraint,'lower')
        [plotDates, plotData] = reduce_to_width(plotTimeStamps,plotCntrlData(:,5),width,[-inf inf]);
        [plotDates2, plotData2] = reduce_to_width(plotTimeStamps,repmat(normCtrlErrorThreshold,size(plotCntrlData(:,5),1),1),width,[-inf inf]);
        plot(plotDates,plotData,'r',plotDates2,plotData2,'g');
        ...
        [plotDates, plotData] = reduce_to_width(plotTimeStamps,plotCntrlData(:,5),width,[-inf inf]);
        [plotDates2, plotData2] = reduce_to_width(plotTimeStamps,repmat(normCtrlErrorThreshold,size(plotCntrlData(:,5),1),1),width,[-inf inf]);
        [plotDates3, plotData3] = reduce_to_width(plotTimeStamps,-repmat(normCtrlErrorThreshold,size(plotCntrlData(:,5),1),1),width,[-inf inf]);
        plot(plotDates,plotData,'r',plotDates2,plotData2,'g',plotDates3,plotData3,'g');
    end
    hold on
    [plotDates, plotData] = reduce_to_width(plotTimeStamps(plotQuality~=goodQualityVal,1),plotCntrlData(plotQuality~=goodQualityVal,5),width,[-inf inf]);
%     plot(plotTimeStamps(plotQuality~=goodQualityVal,1), plotCntrlData(plotQuality~=goodQualityVal,5), 'marker', '.', 'markersize', 10, 'linestyle', 'none', 'color', 'k');
    plot(plotDates, plotData, 'marker', '.', 'markersize', 10, 'linestyle', 'none', 'color', 'k');
    [plotDates, plotData] = reduce_to_width(plotTimeStamps(plotQuality==notRunningQualityVal,1),plotCntrlData(plotQuality==notRunningQualityVal,5),width,[-inf inf]);
%     plot(plotTimeStamps(plotQuality==notRunningQualityVal,1), plotCntrlData(plotQuality==notRunningQualityVal,5), 'marker', '.', 'markersize', 10, 'linestyle', 'none', 'color', [0.93 0.93 0.93]);
    plot(plotDates, plotData, 'marker', '.', 'markersize', 10, 'linestyle', 'none', 'color', [0.93 0.93 0.93]);
    dataL = 0;
    for k = 1:numTS-1
        dataL = dataL + timespanData.length{k};
        if abs(diff([timespanData.timeStamps{k}(end) timespanData.timeStamps{k+1}(1)])) > datenum(0,0,0,0,0,timespanData.timeStep{k})
            plot(repmat(plotTimeStamps(dataL),2,1),yRange_2,'r','LineWidth',2);
        else
            plot(repmat(plotTimeStamps(dataL),2,1),yRange_2,'r--');
        end
    end
    % Add the OPMCondition patches if present
    if ~isempty(condition)
...
%         colList = {'b' 'g' 'r' 'c' 'm' 'y' 'k'};
    ...
    set(gca,'YLim', yRange_2, 'XLim', plotTimeStamps([1, end]));
    if max(get(gca, 'YTick')) > 10000, set(gca,'YTick',get(gca, 'YTick'),'YTickLabel', num2str(get(gca, 'YTick')')); end
    opmdatetick2(gca, 'single',plotTimeStampsActual);
    ylabel('Control error');
    title(sprintf('Combined data (Control error - red; Threshold - green)'));
    grid on;

    result.TimeSpan.Result(1).ImageH = f;
    result.TimeSpan.Result(1).FigureTitle = sprintf('Combined data plot: %s',description);

    % Create combined in-control data (Control error - red/blue; Threshold - green)
    f = opmblankfigure;
    width = get_axes_width(gca(f));
    if strcmpi(reportType,'full')
        result.TimeSpan.Result(2).ImageH = f; % Store blank OPM figure in results
    end
    subplot(3,1,1);
%     subplot(3,3,1:3);
    % Loop through timespans and extract data for normcompare
    for k = 1:numTS
        % Prep data for normcompare
        if plotTimespan(k) == 1
            thisTimeStamp{k} = timespanData.timeStamps{k}(end);
            thisData{k} = timespanData.plotCntrlData{k}(timespanData.timeStamps{k} <= thisTimeStamp{k},5);
            thisInCtrl{k} = timespanData.inCtrlData{k}(timespanData.timeStamps{k} <= thisTimeStamp{k});
            thisData{k} = thisData{k}(thisInCtrl{k}==1);
        else
            thisData{k} = [NaN NaN];
        end
        if size(thisData{k},1) == 0, thisData{k} = [NaN NaN]; end
    end
    hComp = normcompare(thisData{:}, 'Bins', 100);
    % Plot control error thresholds
    hold on
    if strcmpi(controllerConstraint,'upper')
        hTarget = plot(-[normCtrlErrorThreshold normCtrlErrorThreshold], [0; 100], 'g'); set(hTarget, 'Color', [0 1 0], 'LineWidth', 2);
    elseif strcmpi(controllerConstraint,'lower')
        hTarget = plot([normCtrlErrorThreshold normCtrlErrorThreshold], [0; 100], 'g'); set(hTarget, 'Color', [0 1 0], 'LineWidth', 2);
    else
        hTarget = plot([normCtrlErrorThreshold normCtrlErrorThreshold], [0; 100], 'g'); set(hTarget, 'Color', [0 1 0], 'LineWidth', 2);
    ...
    xlabel('Control error');
    axis tight
    if isempty(condition)
        legend(hComp(:,1), sensor(1).Label,'AutoUpdate','off');
    else
        legend(hComp(:,1), condition.legendEntries,'AutoUpdate','off');
    end
    set(gca,'XLim', yRange_2);
    title(sprintf('Combined in-control data (Control error - red/blue; Threshold - green)'));

    % Plot graphs (controller quality and root cause)
    % f = opmblankfigure;
    % if strcmpi(reportType,'full')
    %     result.TimeSpan.Result(3).ImageH = f; % Store blank OPM figure in results
    % end
    subplot(3,1,2);
%     subplot(3,3,4:5);
    [plotDates, plotData] = reduce_to_width(plotTimeStamps,vertcat(timespanData.cntrlQuality{:}),width,[-inf inf]);
%     plot(plotTimeStamps,vertcat(timespanData.cntrlQuality{:}),'b');
    plot(plotDates,plotData,'b');
    hold on;
    yRange_4 = [1-0.1 length(cntrlQualityStrings)+0.1];
    dataL = 0;
    for k = 1:numTS-1
        dataL = dataL + timespanData.length{k};
        if abs(diff([timespanData.timeStamps{k}(end) timespanData.timeStamps{k+1}(1)])) > datenum(0,0,0,0,0,timespanData.timeStep{k})
            plot(repmat(plotTimeStamps(dataL),2,1),yRange_4,'r','LineWidth',2);
        else
            plot(repmat(plotTimeStamps(dataL),2,1),yRange_4,'r--');
        end
    end
    % Add the OPMCondition patches if present
    if ~isempty(condition)
...
%         colList = {'b' 'g' 'r' 'c' 'm' 'y' 'k'};
    ...
    set(gca,'YLim', yRange_4, 'XLim', plotTimeStamps([1, end]));
    if max(get(gca, 'YTick')) > 10000, set(gca,'YTick',get(gca, 'YTick'),'YTickLabel', num2str(get(gca, 'YTick')')); end
%     opmdatetick2(gca, 'single',plotTimeStampsActual);
    opmdatetick2(gca, 'single',plotTimeStampsActual,'numLabels',5);
    set(gca, 'ytick',1:length(cntrlQualityStrings),'yticklabel', cntrlQualityStrings);
    title(sprintf('Combined data: Controller status'));
    grid on;
%     subplot(3,3,6);
%     piData = vertcat(timespanData.cntrlQuality{:});
%     for k = 1:length(cntrlQualityStrings)
%         piPlotData(k) = sum(piData==k);
%     end
%     warning off
%     pie(piPlotData,ones(1,length(piPlotData)),cntrlQualityStrings);
%     warning on
    
    subplot(3,1,3);
%     subplot(3,3,7:8);
    [plotDates, plotData] = reduce_to_width(plotTimeStamps,vertcat(timespanData.rootCause{:}),width,[-inf inf]);
%     plot(plotTimeStamps,vertcat(timespanData.rootCause{:}),'b');
    plot(plotDates,plotData,'b');
    hold on;
    yRange_4 = [1-0.1 length(rootCauseStrings)+0.1];
    dataL = 0;
    for k = 1:numTS-1
        dataL = dataL + timespanData.length{k};
        if abs(diff([timespanData.timeStamps{k}(end) timespanData.timeStamps{k+1}(1)])) > datenum(0,0,0,0,0,timespanData.timeStep{k})
...
%     opmdatetick2(gca, 'single',plotTimeStampsActual);
    opmdatetick2(gca, 'single',plotTimeStampsActual,'numLabels',5);
    set(gca, 'ytick',1:length(rootCauseStrings),'yticklabel', rootCauseStrings);
    title(sprintf('Combined data: Controller root cause'));
    grid on;
%     subplot(3,3,9);
%     piData = vertcat(timespanData.rootCause{:});
%     for k = 1:length(rootCauseStrings)
%         piPlotData(k) = sum(piData==k);
%     end
%     warning off
%     pie(piPlotData,ones(1,length(piPlotData)),rootCauseStrings);
%     warning on
    if strcmpi(reportType,'full')
        result.TimeSpan.Result(2).ImageH = f; % Store figure in results
    end
    % if strcmpi(reportType,'full')
    %     result.TimeSpan.Result(3).ImageH = f; % Store figure in results
    % end
    
    % Create PIE chart of controller STATUS and ROOT CAUSE
    f = opmblankfigure;
    width = get_axes_width(gca(f));
    if strcmpi(reportType,'full')
        result.TimeSpan.Result(3).ImageH = f; % Store blank OPM figure in results
    end
    % Plot controller status PIE graphs
    for i = 1:numTS
        subplot(2,numTS+1,i);
        piData = timespanData.cntrlQuality{i};
        for k = 1:length(cntrlQualityStrings)
            piPlotData(k) = sum(piData==k);
        end
        piPerc = round(piPlotData/sum(piPlotData)*1000)/10;
        piPerc(isnan(piPerc)) = 0;
        piPlotData(piPerc<=0) = 0.001;
        piPlotStrings = cntrlQualityStrings;
        for k = 1:length(cntrlQualityStrings)
%             piPlotStrings{k} = [piPlotStrings{k} ' (' num2str(piPerc(k)) '%)'];
            piPlotStrings{k} = [num2str(piPerc(k)) '%'];
            if piPerc(k) <= 0, piPlotStrings{k} = ''; end
        end
        warning off
        pie(piPlotData,zeros(1,length(piPlotData)),piPlotStrings);
        warning on
        if all(piPlotData==0.001)
            pie(1,{''});
            plotChild = get(gca,'Children');
            for j = 2:2:length(plotChild), plotChild(j).FaceAlpha = 0; end
            set(gca,'Children',plotChild);
        end
        if isempty(condition)
            title(['Control status: ' sensor(1).Label{i}]);
        else
            title(['Control status: ' condition.legendEntries{i}]);
        end
    end
    subplot(2,numTS+1,numTS+1);
    hL = pie(ones(1,length(piPlotData)),zeros(1,length(piPlotData)),repmat({''},1,5));
    legend(cntrlQualityStrings,'orientation','vertical','location','eastoutside','AutoUpdate','off');
    pause(1);
    delete(hL);
%     plotChild = get(gca,'Children');
%     for i = 2:2:length(plotChild), plotChild(i).EdgeColor = [1 1 1]; end
%     for i = 2:2:length(plotChild), plotChild(i).FaceAlpha = 0; end
%     set(gca,'Children',plotChild);
        
    % Plot controller root cause PIE graphs
    for i = 1:numTS
        subplot(2,numTS+1,numTS+1+i);
        piData = timespanData.rootCause{i};
        for k = 1:length(rootCauseStrings)
            piPlotData(k) = sum(piData==k);
        end
        piPerc = round(piPlotData/sum(piPlotData)*1000)/10;
        piPerc(isnan(piPerc)) = 0;
        piPlotData(piPerc<=0) = 0.001;
        piPlotStrings = rootCauseStrings;
        for k = 1:length(rootCauseStrings)
%             piPlotStrings{k} = [piPlotStrings{k} ' (' num2str(piPerc(k)) '%)'];
            piPlotStrings{k} = [num2str(piPerc(k)) '%'];
            if piPerc(k) <= 0, piPlotStrings{k} = ''; end
        ...
        if isempty(condition)
            title(['Control root cause: ' sensor(1).Label{i}]);
        else
            title(['Control root cause: ' condition.legendEntries{i}]);
        end
    end
    subplot(2,numTS+1,2*numTS+2);
    hL = pie(ones(1,length(piPlotData)),zeros(1,length(piPlotData)),repmat({''},1,5));
    legend(rootCauseStrings,'orientation','vertical','location','eastoutside','AutoUpdate','off');
    pause(1);
    delete(hL);
%     plotChild = get(gca,'Children');
%     for i = 2:2:length(plotChild), plotChild(i).EdgeColor = [1 1 1]; end
...
%     set(gca,'Children',plotChild);
    ...
    % Plot graphs (P, I and D tuning parameter)
    if ~isempty(plotTuningData) && ~all(all(isnan(plotTuningData)))
        f = opmblankfigure;
        width = get_axes_width(gca(f));
        if strcmpi(reportType,'full')
            result.TimeSpan.Result(4).ImageH = f; % Store blank OPM figure in results
        end
        for i = 1:size(plotTuningData,2)
            subplot(size(plotTuningData,2),1,i);
            [plotDates, plotData] = reduce_to_width(plotTimeStamps,plotTuningData(:,i),width,[-inf inf]);
%             plot(plotTimeStamps,plotTuningData(:,i));
            plot(plotDates,plotData);
            hold on
            [plotDates, plotData] = reduce_to_width(plotTimeStamps(plotTuningQuality(:,i)~=goodQualityVal),plotTuningData(plotTuningQuality(:,i)~=goodQualityVal,i),width,[-inf inf]);
%             plot(plotTimeStamps(plotTuningQuality(:,i)~=goodQualityVal), plotTuningData(plotTuningQuality(:,i)~=goodQualityVal,i), 'marker', '.', 'markersize', 10, 'linestyle', 'none', 'color', 'k');
            plot(plotDates, plotData, 'marker', '.', 'markersize', 10, 'linestyle', 'none', 'color', 'k');
            [plotDates, plotData] = reduce_to_width(plotTimeStamps(plotTuningQuality(:,i)==notRunningQualityVal),plotTuningData(plotTuningQuality(:,i)==notRunningQualityVal,i),width,[-inf inf]);
%             plot(plotTimeStamps(plotTuningQuality(:,i)==notRunningQualityVal), plotTuningData(plotTuningQuality(:,i)==notRunningQualityVal,i), 'marker', '.', 'markersize', 10, 'linestyle', 'none', 'color', [0.93 0.93 0.93]);
            plot(plotDates, plotData, 'marker', '.', 'markersize', 10, 'linestyle', 'none', 'color', [0.93 0.93 0.93]);
            dataL = 0;
            for k = 1:numTS-1
                dataL = dataL + timespanData.length{k};
        ...
        %         colList = {'b' 'g' 'r' 'c' 'm' 'y' 'k'};
            ...
            set(gca,'XLim', plotTimeStamps([1, end]));
            yRange_1 = [nanmin(plotTuningData(:,i)) nanmax(plotTuningData(:,i))];
            if ~isnan(diff(yRange_1))
                set(gca,'YLim', yRange_1 + [-0.025, 0.025]*max([1 diff(yRange_1)]));
            else
                set(gca,'YLim', [0 1]);
            end
            if max(get(gca, 'YTick')) > 10000, set(gca,'YTick',get(gca, 'YTick'),'YTickLabel', num2str(get(gca, 'YTick')')); end
            ...
            if i == 1
                title('Combined data: P tuning parameter');
            elseif i == 2
                title('Combined data: I tuning parameter');
            elseif i == 3
                title('Combined data: D tuning parameter');
            end
            grid on;
        end
        if strcmpi(reportType,'full')
            result.TimeSpan.Result(4).ImageH = f; % Store figure in results
        end
    end
    ...
    for i = 1:numTS
        treemapParams{i} = ap.AnalysisResultTable('TreemapController');
        try
            ...
            treemapParams{i}.Table = struct(ap.TreemapNodeParams(colourStatus,str2double(newTblCtrl(strcmpi(newTblCtrl,'Unhealthy [%]'),i+1)),unitNameInTree,description));
        catch
...
%% Add the parameters to the results structure
% By default, just call result.addParameters(parser).  However, if the
% code modifies some of the parameters based on the user input or
% dynamically selects which parameters are required, then additional inputs
% 'ModifiedValues' and 'IsRequired' can be added.  The 'ModifiedValues'
% input must be a cell array with a column per parameter and the
% 'IsRequired' input must be a logical array with a column per parameter. 

isRequired = {'description',false};
modifiedValues = {};
try if isempty(t_i ~= parser.Results.tintegral) | (t_i ~= parser.Results.tintegral), modifiedValues = [modifiedValues {'tintegral',t_i}]; end; catch, end
try if isempty(normCtrlErrorThreshold ~= parser.Results.threshold) | (normCtrlErrorThreshold ~= parser.Results.threshold), modifiedValues = [modifiedValues {'threshold',normCtrlErrorThreshold}]; end; catch, end
if exist('parser') && exist('reportType','var') && (strcmpi(reportType,'full') || ~isempty(result.ErrorMsg))
    result.addParameters(parser,'ModifiedValues',modifiedValues,'IsRequired',isRequired);
end