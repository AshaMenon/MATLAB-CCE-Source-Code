function [parameters,inputs, quality, rootCause] =...
    ...
    dataTbl = readtable(fullfile('..','..','data',filenames{1}));
    sensorParameters = readtable(fullfile('..','..','data',filenames{2}));
    parameterTbl = readtable(fullfile('..','..','data',filenames{3}));
    ...
    if any(ismember(varNames, 'sensorDataMV_Values'))
        ...
        inputs.SensorQualityMV = dataTbl.sensorQualityMV_Values;
...
%        sensorQualityMV = [];
    ...
    if any(ismember(varNames, 'sensorDataAuto_Values'))
        ...
        inputs.SensorQualityAutoman = dataTbl.sensorQualityAuto_Values(end);
%     else
%         %sensorDataAutoman =  struct.empty(0,0);
%         sensorDataAutoman =  [];
%          sensorQualityAutoman = [];
    end
    inputs.SensorQualityPV = dataTbl.sensorQualityPV_Values;
    inputs.SensorQualitySP = dataTbl.sensorQualitySP_Values;
