classdef tSystemReenableOnCoord < matlab.unittest.TestCase
    %tSystemReenableOnCoord tests that the coordinator successfully reenables
    % system disabled calcs within coordinator, followed by changing the request enable state. 
    ...
    methods (Test)
        function tReenableSystDisableCalcs(testCase)
            ...
            newElem3.setAttributeValue("CalculationState", "SystemDisabled")
            ...
            %Calc4 - not disabled
            newElem4 = testCase.ElementFolder.addElement("calc4", "Template", 'sensorAdd');
            ...
            expected =  [cce.CalculationState.Idle, cce.CalculationState.Idle,...
                cce.CalculationState.Idle, cce.CalculationState.Idle];
            testCase.verifyEqual([calcState1, calcState2, calcState3, calcState4], expected);

            testCase.Coordinator.refresh;
            requestReenableVal = testCase.Coordinator.getAttributeValue("ReenableSystemDisabledCalcs");
            expected =  false;
            testCase.verifyEqual(requestReenableVal, expected);
