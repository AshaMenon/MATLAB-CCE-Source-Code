classdef CalculationErrorState < uint32
    %CALCULATIONERRORSTATE Enumeration for Calculation Error Digital States
    %   Extends the System Digital State to cater for CCE-specific errors.
    %
    ...
    enumeration
        ...
        DependentInputsNotReady(15) %Calculation's Dependent inputs do not have timestamps available for the checked output time
        NetworkError (16) %Network error while reading inputs
        OutOfMemory (17) %Out of memory error
    ...
    methods
        function tf = isFatal(e)
            %isFatal  True if the error state must result in a calculation being disabled
            fatalStates = [...
            cce.CalculationErrorState.CalculationServerInvalid, ...
            cce.CalculationErrorState.CalculationNameUnknown, ...
            cce.CalculationErrorState.ExecutionFrequencyInvalid, ...
            cce.CalculationErrorState.LogNameInvalid, ...
            cce.CalculationErrorState.CalculationStateInvalid, ...
            cce.CalculationErrorState.LastCalcTimeInvalid, ...
            cce.CalculationErrorState.InputConfigInvalid, ...
            cce.CalculationErrorState.InputTimeRangeInvalid, ...
            cce.CalculationErrorState.OutputConfigInvalid, ...
            cce.CalculationErrorState.UnhandledException, ...
            cce.CalculationErrorState.CircularDependencyChain, ...
            cce.CalculationErrorState.RetiredCalculationDependency, ...
            cce.CalculationErrorState.ConfigurationError, ...
            cce.CalculationErrorState.CalcFailed];
            tf = ismember(e, fatalStates);
        end
        function tf = isQuestionable(e)
            %isQuestionable  True if the error state means the value should not be trusted
            questionableStates = [...
            cce.CalculationErrorState.NoResult, ...
            cce.CalculationErrorState.NoLabData, ...
            cce.CalculationErrorState.NoData, ...
            cce.CalculationErrorState.UnderRange, ...
            cce.CalculationErrorState.OverRange, ...
            cce.CalculationErrorState.BadInput, ...
            cce.CalculationErrorState.Bad, ...
            cce.CalculationErrorState.CommFail];
            tf = ismember(e, questionableStates);
        end
    end
end
