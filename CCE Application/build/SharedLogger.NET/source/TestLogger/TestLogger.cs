...
namespace TestLogger
{
    class TestLogger
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Press any key to test environment variable version...");
            Console.ReadKey();

            Console.WriteLine("Testing environment variable option.");
            Environment.SetEnvironmentVariable("LoggerTestEnv", @"D:\Scratch");

            string fileNameEnv = @"NetLogfileEnv.txt";
            Console.WriteLine("Writing output to {0} using environment variable 'LoggerTestEnv'", fileNameEnv);
            Logger loggerEnv = new Logger(fileNameEnv, "", "Test", LogMessageLevel.All, "LoggerTestEnv");
            Console.WriteLine("Logging some messages in ALL level.");
            loggerEnv.logInfo("TestLogger application started.");
            Console.WriteLine("Log file written. Opening file in default application.");
            System.Diagnostics.Process.Start(Environment.GetEnvironmentVariable("LoggerTestEnv") + @"\" + fileNameEnv);
            Environment.SetEnvironmentVariable("LoggerTestEnv", "");
