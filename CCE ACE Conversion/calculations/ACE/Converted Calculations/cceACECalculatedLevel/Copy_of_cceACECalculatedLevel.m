    ...
    try
        ...
        try % Measured stock
            ...
            try
                ...
                MeasStock = aliasDataGet(measInputs, ExeTime, Tol, inputs, logger);
            catch ex
        ...
        try %Surveyed Stock level
            ...
            try %Get Surveyed Stock level
                ...
                SurveStock = aliasDataGet(survInputs, ExeTime, Tol, inputs, logger);
                logger.logTrace("Survey: " + string(SurveStock))
                logger.logTrace("Survey length:" + length(SurveStock))
            ...
            try
                if SurveStock > 0 % Got surveyed values, get PlantMeasures
                    SurvePlantMeasures = aliasDataGet(survPlantMeasInputs, ExeTime, Tol, inputs, logger);

                    if isnan(SurvePlantMeasures) || isempty(SurvePlantMeasures) || ismissing(SurvePlantMeasures)
                        SurvePlantMeasures = 0;
                    end

                    SurveStock = SurveStock + SurvePlantMeasures;
                    outputs.SurveyedStockLevel = SurveStock;
                else
                    outputs.SurveyedStockLevel = nan;
                end
            catch ex
        ...
        try % Theoretical stock level
            ...
            if SurveStock > 0 % Got a Surveyed stock figure therefore use it
                newStock = SurveStock;
            else
                ...
                try %Previous Stock level
                    ...
                    AddTot = aliasDataGet(addInputs, ExeTime, Tol, inputs, logger);
                    SubTot = aliasDataGet(subInputs, ExeTime, Tol, inputs, logger);

                    newStock = PrevStock + AddTot - SubTot;
                catch ex
