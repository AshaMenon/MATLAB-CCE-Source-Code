...
{
    public class APLetheAverage : LetheCalculation
    {
        ...
        /// 
        /// </summary>
        public APLetheAverage() : base() { }

        /// <summary>
        /// construct calc
        /// </summary>
        /// <param name="CalculationElement"></param>
        public APLetheAverage(AFElement CalculationElement) : base(CalculationElement) { }
        ...
        public override void CalculationInitialize()
        {
            try
            {
                ...
                Log.Info("Calculation Average specific initialization starting for:'{0}'", Element.GetPath());
            ...
            {
                Log.Fatal(e, "Calculation Average Initialize Error on '{0}'. Message: {1} ", Element.GetPath(), e.Message);
                throw new Exception("Calculation Average Initialization error",e.InnerException);
        ...
        public override AFValues Evaluate(DateTime CalculationTime)
        {
                ...
                foreach (AFTime tim in outputTimeList)
                {
                    ...
                    try
                    {
                       ...
                       // SearchTimes.Add(DataStartTime.LocalTime);
                        ...
                        //to account for averages ahead i.e daterange is negative, check date and use a min max date.
                       ...
                       // runVals = colVals.Where(t => t.Item1 >= (SearchTimes.Min()).LocalTime && t.Item1 <= (SearchTimes.Max()).LocalTime).ToList();
                        ...
                        if (runVals != null)
                        {
                            ...
                            if (runVals.Count > 0)
                            {
                                ...
                                double Total = runVals.Select(t => t.Item2).Average();
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    Log.Error("Calculation Average Error on '{0}'. no good results from '{1}' ", Element.GetPath(), tim.LocalTime.ToString());
                                }
                        ...
                        else
                        {
                            ...
                            Log.Error("Calculation Average Error on '{0}'. Estimates had a null result set from '{1}' ", Element.GetPath(), tim.LocalTime.ToString());
                    ...
                    {
                        Log.Fatal(e, "Calculation Average loop Error on '{0}'. for calculation time '{1}' ", Element.GetPath(), tim.LocalTime.ToString());
            ...
            {
                Log.Fatal(e, "Calculation Average Error on '{0}'. Message: {1} ", Element.GetPath(), e.Message);
            }
