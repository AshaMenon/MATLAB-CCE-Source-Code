function [outputs, errorCode] = netFunctionTemplate(parameters, inputs)
    %UNTITLED Summary of this function goes here
    %   Detailed explanation goes here
    
    try
        % Get path of calculation
        filePath = which('calculationTemplate.dll');
        if isempty(which("calculationTemplate.Class1"))
            asm = NET.addAssembly(filePath); %#ok<NASGU>
        end     
        
        % Create instance of class and set inputs and parameters
        calcInstance = calculationTemplate.Class1();
        
        % Assign Parameters
        netParams = calculationTemplate.Parameters();
        ...
        % Assign Inputs
        netInputs = calculationTemplate.Inputs();
