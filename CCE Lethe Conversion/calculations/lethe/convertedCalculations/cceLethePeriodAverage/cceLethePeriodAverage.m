function [outputs, errorCode] = cceLethePeriodAverage(parameters, inputs)
    %UNTITLED Summary of this function goes here
    %   Detailed explanation goes here
    
    try
        % Get path of calculation
        filePath = which('cceLethePeriodAverage.dll');
        if isempty(which("cceLethePeriodAverage.cceLethePeriodAverageClass"))
            asm = NET.addAssembly(filePath); %#ok<NASGU>
        end     
        
        % Create instance of class and set inputs and parameters
        calcInstance = cceLethePeriodAverage.cceLethePeriodAverageClass();
        
        % Assign Parameters
        netParams = cceLethePeriodAverage.Parameters();
        ...
        % Assign Inputs
        netInputs = cceLethePeriodAverage.Inputs();
...
if size(outVal,2) == 1
    ...
    outVal = [outVal, null(length(outVal),1)];
end
end
