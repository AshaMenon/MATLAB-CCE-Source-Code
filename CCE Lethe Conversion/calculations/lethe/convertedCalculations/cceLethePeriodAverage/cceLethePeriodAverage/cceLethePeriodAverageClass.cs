...
namespace cceLethePeriodAverage
{
    ...
    public class cceLethePeriodAverageClass
    {
        // Log Info
        ...
        public Outputs RunCalc(Parameters sParams, Inputs sInputs)
        {
            ...
            try
            {
                ...
                foreach (DateTime calcTime in dateRange)
                {
                    ...
                    {
                        double aggregateValue = GoodInputsInPeriod.Average();
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            //double numInputs = sInputs.AdditionalInputs.GetLength(1);
                            //AggregateList.Add(numInputs);
        ...
        public static double[] GetGoodItemsInPeriod(DateTime calcTime, bool ForceTimeCollation, Inputs sInputs)
        {
            ...
            if (idxArray.Contains(-1))
            {
                ...
                {
                    try { outputArray[0] = sInputs.Input[InputIdx]; } catch { }
                    if (numAdditionalInputs > 0)
                    {
                        double[] values;
                        ...
                        for (int colIdx = 1; colIdx <= numAdditionalInputs; colIdx++)
                        {
                            ...
                            try { outputArray[colIdx] = values[idxArray[colIdx]]; } catch { }
                        }
                    }
                }
