function [outputs, errorCode] = cceLetheBUH(parameters, inputs)
    %UNTITLED Summary of this function goes here
    %   Detailed explanation goes here
    
    try
        % Get path of calculation
        filePath = which('cceLetheBUH.dll');
        if isempty(which("cceLetheBUH.cceLetheBUHClass"))
            asm = NET.addAssembly(filePath); %#ok<NASGU>
        end     
        
        % Create instance of class and set inputs and parameters
        calcInstance = cceLetheBUH.cceLetheBUHClass();
        
        % Assign Parameters
        netParams = cceLetheBUH.Parameters();
