function [outputs, errorCode] = cceLetheMassPull(parameters, inputs)
    %UNTITLED Summary of this function goes here
    %   Detailed explanation goes here
    
    try
        % Get path of calculation
        filePath = which('cceLetheMassPull.dll');
        if isempty(which("cceLetheMassPull.cceLetheMassPullClass"))
            asm = NET.addAssembly(filePath); %#ok<NASGU>
        end     
        
        % Create instance of class and set inputs and parameters
        calcInstance = cceLetheMassPull.cceLetheMassPullClass();
        
        % Assign Parameters
        netParams = cceLetheMassPull.Parameters();
        ...
        % Assign Inputs
        netInputs = cceLetheMassPull.Inputs();
