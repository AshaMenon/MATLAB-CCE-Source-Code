function [outputs, errorCode] = cceLetheAverage(parameters, inputs)
    %UNTITLED Summary of this function goes here
    %   Detailed explanation goes here
    
    try
        % Get path of calculation
        filePath = which('cceLetheAverage.dll');
        if isempty(which("cceLetheAverage.cceLetheAverageClass"))
            asm = NET.addAssembly(filePath); %#ok<NASGU>
        end     
        
        % Create instance of class and set inputs and parameters
        calcInstance = cceLetheAverage.cceLetheAverageClass();
        
        % Assign Parameters
        netParams = cceLetheAverage.Parameters();
        ...
        % Assign Inputs
        netInputs = cceLetheAverage.Inputs();
