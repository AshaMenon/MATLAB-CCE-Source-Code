function [outputs, errorCode] = cceLetheControlledSubstitute(parameters, inputs)
    %cceLetheControlledSubstitute Selects the highest-priority substitute
    %   signal for a given calculation period, subject to a FeedDrymass
    %   interlock and an operator override.
    %
    %   Priority is determined by the numeric suffix of each candidate signal
    %   name (lower suffix = higher priority).  When OverrideAction == 1 the
    %   UserOverride value is used instead of the normal candidate selection.
    %   If FeedDrymass is absent or <= 0 the output is NaN for that period.
    %
    %   RollupInputs (set in parameters) should list the candidate signals
    %   that participate in priority-ordered selection:
    %       ["Estimate", "UserEstimate", "Input", "Ma", "Mog", "Minpas"]
    %
    %   Non-rollup inputs (supplied as scalar arrays with matching Timestamps):
    %       FeedDrymass, OverrideAction, OverrideSelection, UserOverride

    try
        % Get path of calculation
        filePath = which('cceLetheControlledSubstitute.dll');
        if isempty(which("cceLetheControlledSubstitute.cceLetheControlledSubstituteClass"))
            asm = NET.addAssembly(filePath); %#ok<NASGU>
        end

        % Create instance of class and set inputs and parameters
        calcInstance = cceLetheControlledSubstitute.cceLetheControlledSubstituteClass();

        % Initialise inputs
        for rollUp = parameters.RollupInputs
            inputs.(rollUp + "Timestamps") = NaT;
        end

        % Ensure that all input names have first letter capitalised
        inputFieldnames = string(fieldnames(inputs));
        inputFieldnames(contains(inputFieldnames,"Timestamp")) = [];
        inputsTemp = struct;

        for fieldName = inputFieldnames'
            newFieldName = capitaliseFirstLetter(fieldName);
            inputsTemp.(newFieldName) = inputs.(fieldName);
            inputsTemp.(newFieldName+"Timestamps") = inputs.(fieldName+"Timestamps");
        end

        inputs = inputsTemp;
        inputFieldnames = string(fieldnames(inputs));

        % Ensure all necessary inputs are included
        rollUpIns = parameters.RollupInputs;
        for n=1:length(rollUpIns)
            if ~contains(inputFieldnames,rollUpIns(n))
                inputs.(rollUpIns(n)) = NaN;
                inputs.(rollUpIns(n)+"Timestamps") = NaT;
            end
        end

        % Handle appended digits
        inputFieldnames = string(fieldnames(inputs));
        inputFieldnames(contains(inputFieldnames,"Timestamp")) = [];

        for fieldName = inputFieldnames'
            if contains(fieldName,"_")
                newFieldName = extractBefore(fieldName, "_");
                inputs.(newFieldName + "Timestamps") = inputs.(fieldName + "Timestamps");
            end
        end

        % Assign Parameters
        netParams = cceLetheControlledSubstitute.Parameters();
        ...
        % Assign Inputs
        netInputs = cceLetheControlledSubstitute.Inputs();
...
function newStr = capitaliseFirstLetter(str)
    str=lower(str);
    expression = '(^|\.)\s*.';
    replace = '${upper($0)}';
