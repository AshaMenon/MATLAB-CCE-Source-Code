function [outputs, errorCode] = cceLetheComponentArray(parameters, inputs)
    %UNTITLED Summary of this function goes here
    %   Detailed explanation goes here
    
    try
        % Get path of calculation
        filePath = which('cceLetheComponentArray.dll');
        if isempty(which("cceLetheComponentArray.cceLetheComponentArrayClass"))
            asm = NET.addAssembly(filePath); %#ok<NASGU>
        end     
        
        % Create instance of class and set inputs and parameters
        calcInstance = cceLetheComponentArray.cceLetheComponentArrayClass();
        
        % Assign Parameters
        netParams = cceLetheComponentArray.Parameters();
        ...
        % Assign Inputs
        netInputs = cceLetheComponentArray.Inputs();
...
function [valArray, suffixStr] = rollupInput(prefixString, availInputs)
    ...
    if ~isempty(useFields)
        ...
        for iField = 1:numel(useFields)
            lenField = length(availInputs.(useFields{iField}));
            valArray(1:lenField, iField) = availInputs.(useFields{iField});
