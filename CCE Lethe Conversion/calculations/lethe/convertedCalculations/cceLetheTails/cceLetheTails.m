function [outputs, errorCode] = cceLetheTails(parameters, inputs)
    %UNTITLED Summary of this function goes here
    %   Detailed explanation goes here
    
    try
        % Get path of calculation
        filePath = which('cceLetheTails.dll');
        if isempty(which("cceLetheTails.cceLetheTailsClass"))
            asm = NET.addAssembly(filePath); %#ok<NASGU>
        end     
        
        % Create instance of class and set inputs and parameters
        calcInstance = cceLetheTails.cceLetheTailsClass();
        
        % Assign Parameters
        netParams = cceLetheTails.Parameters();
        ...
        % Assign Inputs
        netInputs = cceLetheTails.Inputs();
