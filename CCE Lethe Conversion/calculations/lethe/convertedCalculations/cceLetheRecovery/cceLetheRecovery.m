function [outputs, errorCode] = cceLetheRecovery(parameters, inputs)
    %UNTITLED Summary of this function goes here
    %   Detailed explanation goes here
    
    try
        % Get path of calculation
        filePath = which('cceLetheRecovery.dll');
        if isempty(which("cceLetheRecovery.cceLetheRecoveryClass"))
            asm = NET.addAssembly(filePath); %#ok<NASGU>
        end     
        
        % Create instance of class and set inputs and parameters
        calcInstance = cceLetheRecovery.cceLetheRecoveryClass();
        
        % Assign Parameters
        netParams = cceLetheRecovery.Parameters();
        ...
        % Assign Inputs
        netInputs = cceLetheRecovery.Inputs();
