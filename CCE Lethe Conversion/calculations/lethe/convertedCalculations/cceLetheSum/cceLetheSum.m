function [outputs, errorCode] = cceLetheSum(parameters, inputs)
    %UNTITLED Summary of this function goes here
    %   Detailed explanation goes here
    
    try
        % Get path of calculation
        filePath = which('cceLetheSum.dll');
        if isempty(which("cceLetheSum.cceLetheSumClass"))
            asm = NET.addAssembly(filePath); %#ok<NASGU>
        end     
        
        % Create instance of class and set inputs and parameters
        calcInstance = cceLetheSum.cceLetheSumClass();
        
        % Assign Parameters
        netParams = cceLetheSum.Parameters();
