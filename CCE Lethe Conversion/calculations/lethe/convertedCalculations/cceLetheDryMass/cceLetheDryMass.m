function [outputs, errorCode] = cceLetheDryMass(parameters, inputs)
    %UNTITLED Summary of this function goes here
    %   Detailed explanation goes here
    
    try
        % Get path of calculation
        filePath = which('cceLetheDryMass.dll');
        if isempty(which("cceLetheDryMass.cceLetheDryMassClass"))
            asm = NET.addAssembly(filePath); %#ok<NASGU>
        end     
        
        % Create instance of class and set inputs and parameters
        calcInstance = cceLetheDryMass.cceLetheDryMassClass();
        
        % Assign Parameters
        netParams = cceLetheDryMass.Parameters();
        ...
        % Assign Inputs
        netInputs = cceLetheDryMass.Inputs();
