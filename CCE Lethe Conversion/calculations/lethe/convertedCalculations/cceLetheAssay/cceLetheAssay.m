function [outputs, errorCode] = cceLetheAssay(parameters, inputs)
    %UNTITLED Summary of this function goes here
    %   Detailed explanation goes here
    
    try
        % Get path of calculation
        filePath = which('cceLetheAssay.dll');
        if isempty(which("cceLetheAssay.cceLetheAssayClass"))
            asm = NET.addAssembly(filePath); %#ok<NASGU>
        end     
        
        % Create instance of class and set inputs and parameters
        calcInstance = cceLetheAssay.cceLetheAssayClass();
        
        % Assign Parameters
        netParams = cceLetheAssay.Parameters();
        ...
        % Assign Inputs
        netInputs = cceLetheAssay.Inputs();
