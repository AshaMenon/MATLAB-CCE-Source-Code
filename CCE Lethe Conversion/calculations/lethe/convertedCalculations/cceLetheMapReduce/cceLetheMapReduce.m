function [outputs, errorCode] = cceLetheMapReduce(parameters, inputs)
    %UNTITLED Summary of this function goes here
    %   Detailed explanation goes here
    
    try
        % Get path of calculation
        filePath = which('cceLetheMapReduce.dll');
        if isempty(which("cceLetheMapReduce.cceLetheMapReduceClass"))
            asm = NET.addAssembly(filePath); %#ok<NASGU>
        end     
        
        % Create instance of class and set inputs and parameters
        calcInstance = cceLetheMapReduce.cceLetheMapReduceClass();
        
        % Assign Parameters
        netParams = cceLetheMapReduce.Parameters();
        ...
        % Assign Inputs
        netInputs = cceLetheMapReduce.Inputs();
