function [outputs, errorCode] = cceLethePeriodSum(parameters, inputs)
    %UNTITLED Summary of this function goes here
    %   Detailed explanation goes here
    
    try
        % Get path of calculation
        filePath = which('cceLethePeriodSum.dll');
        if isempty(which("cceLethePeriodSum.cceLethePeriodSumClass"))
            asm = NET.addAssembly(filePath); %#ok<NASGU>
        end     
        
        % Create instance of class and set inputs and parameters
        calcInstance = cceLethePeriodSum.cceLethePeriodSumClass();

        % Handle cases where only additional inputs have data
        % 
        % if any(inputs.InputTimestamps == NaT) && ~isempty(parameters.AdditionalInputs)
        %     additionalInputName = parameters.AdditionalInputs(1);
        % 
        %     inputs.InputTimestamps = inputs.(additionalInputName + "Timestamps");
        %     inputs.Input = zeros(size(inputs.InputTimestamps));
        % end
        
        % Assign Parameters
        netParams = cceLethePeriodSum.Parameters();
        ...
        if isfield(parameters, "AdditionalInputs") && ~isempty(parameters.AdditionalInputs)
            ...
            additionalInputFields = matlab.lang.makeValidName(additionalInputFields);
        else
            additionalInputFields = string([]);
        end
        
        % Assign Inputs
        netInputs = cceLethePeriodSum.Inputs();
        ...
        if ~isempty(additionalInputFields)
            ...
            netInputs.AdditionalInputs = [NaN,NaN];
            netInputs.AdditionalTimestamps = ...
                parseMatlabDateTime([datetime("1970-01-01"),datetime("1970-01-01")]);
        end
...
outTime = repmat(datetime("1970-01-01 00:00:00"), max(arraySizes), length(inputFields));
colIdx = 1;
...
natIdx = isnat(outTime);
outTime(natIdx) = datetime("1970-01-01 00:00:00");

% ensure that outVal has more than 1 column as calc is expecting a double[,]
if size(outVal,2) == 1
    outVal = [outVal, zeros(length(outVal),1)];
