function [outputs, errorCode] = cceLetheComponent(parameters, inputs)
    %UNTITLED Summary of this function goes here
    %   Detailed explanation goes here
    
    try
        % Get path of calculation
        filePath = which('cceLetheComponent.dll');
        if isempty(which("cceLetheComponent.cceLetheComponentClass"))
            asm = NET.addAssembly(filePath); %#ok<NASGU>
        end     
        
        % Create instance of class and set inputs and parameters
        calcInstance = cceLetheComponent.cceLetheComponentClass();
        
        % Assign Parameters
        netParams = cceLetheComponent.Parameters();
        ...
        % Assign Inputs
        netInputs = cceLetheComponent.Inputs();
