function [outputs, errorCode] = ccePyKilkenProduction(parameters,inputs)
try
    ...
    % write it out as .xml
    writetable(struct2table(inputs), fullfile('D:','CCE Dependencies','CCE DataScienceModels', 'PythonFiles', 'inputsKilken.csv'));
    writetable(struct2table(parameters), fullfile('D:','CCE Dependencies','CCE DataScienceModels', 'PythonFiles', 'parametersKilken.csv'));

    % run python externally
    [status, sysOut] = system('python "D:/CCE Dependencies/CCE DataScienceModels/PythonFiles/CCEScripts/RunPyKilken.py"');
%     [status, sysOut] = system('python ./CCEScripts/RunPyBasicity.py');

    % Check it ran successfully
    if ~status
        % read python generated output file
        outputs = readtable(fullfile('D:', 'CCE Dependencies','CCE DataScienceModels', 'PythonFiles', 'outputsKilken.csv'));
        outputs = table2struct(outputs);
        outputs = rmfield(outputs, "Var1");
        errorCode = readlines(fullfile('D:', 'CCE Dependencies','CCE DataScienceModels','PythonFiles', 'errorKilken.txt'));
