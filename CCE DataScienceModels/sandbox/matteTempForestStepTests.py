...
from examples import exampleMatteTempModellingForestPipeline as forest
scaledPredictors = forest.scaler.fit_transform(predictors)
predictedMatteTemp = forest.forestMdl.predict(scaledPredictors)
...
axs[0].title.set_text('Step Tests (Actual Data) - Forest Model')
...
fig, axs = plt.subplots(2,sharex=True)
axs[0].title.set_text('Step Tests - Forest Model')
...
scaledPredictors = forest.scaler.fit_transform(predictors)
predictedMatteTemp2 = forest.forestMdl.predict(scaledPredictors)

# Find difference in predicted matte temperatures
predictedMatteTempDiff2 = np.diff(predictedMatteTemp2)
fuelCoalRateDiff2 = stepData2["Fuel coal feed rate"].diff()
...
scaledPredictors = forest.scaler.fit_transform(predictors)
predictedMatteTemp3 = forest.forestMdl.predict(scaledPredictors)
...
stepChange2["Fuel coal feed rate"] = stepChange2["Fuel coal feed rate"].values + (stepChange2["Fuel coal feed rate"] * -0.2)
...
stepChange2['Lance air flow rate PV'] = lanceAir + (fuelCoalAir1 - stepChange2['Lance coal carrier air'])


# Use step data to get predictions
predictors = stepData4[predictorTags]

scaledPredictors = forest.scaler.fit_transform(predictors)
predictedMatteTempBaseline = forest.forestMdl.predict(scaledPredictors)


predictors = stepChange1[predictorTags]
scaledPredictors = forest.scaler.fit_transform(predictors)
predictedMatteTemp1 = forest.forestMdl.predict(scaledPredictors)

predictors = stepChange2[predictorTags]
scaledPredictors = forest.scaler.fit_transform(predictors)
predictedMatteTemp2 = forest.forestMdl.predict(scaledPredictors)
...
fig, axs = plt.subplots(2,sharex=True)
axs[0].title.set_text('Step Tests - Forest Model')
stepData4['Fuel coal feed rate'].plot(ax = axs[0], color = 'b')
stepChange1['Fuel coal feed rate'].plot(ax = axs[0], color = 'g')
stepChange2['Fuel coal feed rate'].plot(ax = axs[0], color = 'r')
