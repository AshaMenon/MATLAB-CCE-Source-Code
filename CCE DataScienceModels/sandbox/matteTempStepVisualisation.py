# -*- coding: utf-8 -*-
"""
Step tests Temp model

Analyzing the relationship between controlled and manipulated variables
"""
...
def processingFunc(fullDFOrig, predictorTags, highFreqPredictors, lowFreqPredictors):
    ...
    fullDF, origSmoothedResponses, predictorTagsNew = \
        prep.preprocessingAndFeatureEngineering(
            ...
            addRollingSumPredictors={'add': True, 'window': '100min', 'on': highFreqPredictors}, #NOTE: functionality exists to process an 'on' key
            ...
            addShiftsToPredictors={'add': True, 'nLags': 3, 'on': ['Fuel coal feed rate PV 100min-rollingmean', 'Fuel coal feed rate PV 100min-rollingsum',
                                                                   'Matte feed PV 100min-rollingmean', 'Matte feed PV 100min-rollingsum',
                                                                   'Roof matte feed rate PV 100min-rollingmean', 'Roof matte feed rate PV 100min-rollingsum']},
...
maxTrainSize = int(21*24*60/100)
testSize = int(1*24*60/100)
...
param = {
    # 'regression__alpha': np.logspace(-5, 4, num = 10000),
    # 'poly__degree': np.arange(1, 6, 1),
    'regression__n_components': np.arange(2, len(predictorTagsNew), 1),
    }
...
k = searchResults.best_params_.get('regression__n_components')
...
predictorWeights = regressionCoefs.ravel()
...
#%% Step test
def stepTest(start_date, end_date, stepSize, heading, mvTag):
    ...
    #sim.createStepTestPlots(sim_run, responseTags, mvTag + ' 100min-rollingsum', heading)
    return sim_run

#%% Step Test Control Variable vs. Manipulated Variable
...
# averageTempStepChange
# averageFuelFeedRateStepChange
startStepSize = -60
endStepSize = 60

#Loops through the numerous step sizes in fuel feed rate, simulate the linear model and plots the resulting temperature changes
for stepSize in range(startStepSize, endStepSize, 10):                 
    stepSize = np.array(stepSize*0.01)
    
    sim_run = stepTest(start_date, end_date, stepSize, heading, mvTag)
    
    # Extract the baseline and predicted values
    baselineResponse = sim_run['baselineResponse']
    stepResponse = sim_run['stepResponse']
        
    averageTempStepChange = [statistics.mean((stepResponse - baselineResponse).flatten())]
    #percentageDifference = statistics.mean(((baselineResponse - stepResponse)/baselineResponse)*100)
    
    baselineMV = sim_run['baselinePredictors'][mvTag + ' 100min-rollingsum']
    stepMV = sim_run['stepPredictors'][mvTag + ' 100min-rollingsum']
    
    averageFuelFeedRateStepChange = [statistics.mean(stepMV - baselineMV)]          #We need to standardise the data type between the response variables and the MV 
    #percentageDifferenceMV = statistics.mean(((baselineMV - stepMV)/baselineMV)*100)
    
    plt.plot(averageFuelFeedRateStepChange,averageTempStepChange, "*")
    plt.xlabel("Fuel coal feed rate [tons/hour]")
    plt.ylabel("Matte Temperatures [°C]")

#%% Box plot of Step Test Control Variable vs. Manipulated Variable
...
for stepSize in range(startStepSize, endStepSize, 10):                 
    ...
    DFstepMV["%.0f" % (stepSize*100)+'%'] = stepMV
    
DFstepMV[list(DFstepMV.columns)].plot(kind='box')
