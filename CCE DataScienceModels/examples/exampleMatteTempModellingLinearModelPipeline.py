...
def processingFunc(fullDFOrig, predictorTags, highFreqPredictors, lowFreqPredictors):
    ...
    fullDF, origSmoothedResponses, predictorTagsNew = \
        prep.preprocessingAndFeatureEngineering(
            ...
            addShiftsToPredictors={'add': False, 'nLags': 3, 'on': ['Fuel coal feed rate PV 15min-rollingmean',
                                                                   'Matte feed PV 15min-rollingmean',
                                                                   'Roof matte feed rate PV 15min-rollingmean',]},
...
ax = visualise.plotDirectionalPerformance(origResponsesTest, testResults, convergenceIndicator, "Linear Model - Temperature (Test Results)")
predictorsTestPrepended['Matte temperatures 450min-rollingmean'].plot(ax = ax, style = 'k--')
