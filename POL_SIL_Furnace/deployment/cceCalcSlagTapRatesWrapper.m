...
try
    ...
    parametersDictionary = loadConstantsFromDictionary(slddPath);
    parameters = appendStructs(parameters, parametersDictionary);

    dataTT = preprocessDataForSlagTapping(dataTT,parameters);

    [outputs.SlagTapRates, outputs.CalibrationFactor] = calcSlagTapRates(dataTT, parameters, log);
    outputs.Timestamp = dataTT.Timestamp;
    outputs.SlagTapRates = outputs.SlagTapRates((end-parameters.ExecutionFrequencyParam+1):end);
    outputs.CalibrationFactor = outputs.CalibrationFactor((end-parameters.ExecutionFrequencyParam+1):end);
    outputs.Timestamp = dataTT.Timestamp((end-parameters.ExecutionFrequencyParam+1):end);
