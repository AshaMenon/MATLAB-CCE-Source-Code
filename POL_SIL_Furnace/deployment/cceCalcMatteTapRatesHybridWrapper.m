function [outputs, errorCode] = cceCalcMatteTapRatesHybridWrapper(parameters, inputs)
 ...
 % Create log
    ...
    log.logInfo('--------------------------------------------------------------')
    log.logInfo('Start matte tap rate calculation at execution time %s', string(exeTime))

    try
        % [Extract inputs]
        % Data format checking and converting
        inputsTT = convertInputs(inputs, log);
        matteTapRatesTonPerHr = calcMatteTapRatesHybridDeployment(inputsTT, parameters.DefaultLadleWeightTon, parameters.MinimumTapDurationMins, parameters.ExecutionFrequencyParam);
        outputs.MatteTapRatesTonPerHr = matteTapRatesTonPerHr(end-parameters.ExecutionFrequencyParam+1:end);
...
function outputs = assignNans(inputs, log)
    ...
    outputs.MatteTapRatesTonPerHr = nan;
end