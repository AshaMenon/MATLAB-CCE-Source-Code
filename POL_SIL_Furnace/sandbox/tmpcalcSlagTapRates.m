function slagTapRate = calcSlagTapRates(dataTT, parameters)
...
moistureContent = calculateMoistureContent(dataTT, parameters, slagTapRate);

% Adjust tapped slag for moisture
slagTapRate = slagTapRate .* (1 - moistureContent);
end
...
%periodHolder = [];
% Loop through data in steps of 'calibrationPeriod' until reaching the end.
% 
% inputsTT.NewMeanSlagThickness(isnan(inputsTT.NewMeanSlagThickness)) =...
%     inputsTT.SlagThickness(isnan(inputsTT.NewMeanSlagThickness));
% 
% inputsTT.NewMeanConcThickness(isnan(inputsTT.NewMeanConcThickness)) =...
%     inputsTT.ConcThickness(isnan(inputsTT.NewMeanConcThickness));
...
while startIdx <= endIdx && periodEndIdx ~= endIdx
    ...
    commonValidSoundings = find(inputsTT.CombinedValidDeltaSounding(startIdx : periodEndIdx)) + startIdx - 1;

    % Find the last common sounding of the period.
    if ~isempty(commonValidSoundings)
    lastSoundingIndex = commonValidSoundings(end);
    else
        lastSoundingIndex = periodEndIdx;
    end

    % Calculate the flow rates and accumulation for this period.

    feedInBetweenSoundings = sum(inputsTT.FeedTonPerHr(startIdx : lastSoundingIndex)) / 60;
    offGasOutBetweenSoundings = feedInBetweenSoundings * parameters.offgas_fraction;
    slagOutBetweenSoundings = sum(slagTapRate(startIdx:lastSoundingIndex)) / 60;
    matteOutBetweenSoundings = sum(inputsTT.MatteTapRatesTonPerHr(startIdx:lastSoundingIndex)) / 60;

    slagHeightChange = (inputsTT.NewMeanSlagThickness(lastSoundingIndex) - inputsTT.NewMeanSlagThickness(startIdx)) / 100;
    ...
    matteHeightChange = (inputsTT.NewMeanMattePlusBuildupThickness(lastSoundingIndex) - ...
        inputsTT.NewMeanMattePlusBuildupThickness(startIdx)) / 100;

    matteAccumulated = matteHeightChange * parameters.bath_area * parameters.matte_density;

    concHeightChange = (inputsTT.NewMeanConcThickness(lastSoundingIndex) - inputsTT.NewMeanConcThickness(startIdx)) / 100;
    ...
    % Account for negative or excessively high moisture values. Otherwise a default value is used.
    % if moistureContentBetweenSoundings < 0 || moistureContentBetweenSoundings > 0.9
    %     moistureContentBetweenSoundings =  parameters.SlagConveyorMoisture;
    % end
...
% moistureValidation = moistureContent;
% for i = 1:(size(periodHolder, 1) - 1)
%     moistureValidation(periodHolder(i) : periodHolder(i + 1)) = moistureContent(periodHolder(i) - 5);
% end
% moistureContent = moistureValidation;
end
