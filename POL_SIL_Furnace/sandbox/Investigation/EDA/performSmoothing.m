function smoothedData = performSmoothing(dataTable, monthIndex, smoothingFactor)

% Calculate the durations between consecutive datetime values
% Choose a reference datetime (e.g., the first datetime in the array)
referenceDatetime = dataTable.Timestamp(1);

% Calculate the durations by subtracting the reference datetime
durationVector = years(dataTable.Timestamp - referenceDatetime);
